                    ...
                    <tbody>
                        @foreach ($kayu as $item)
                            ...
                            <div id="detailPopup"
                                ...
                                <div class="bg-white p-4 md:p-6 rounded-lg shadow-lg w-96 max-w-full">
                                    <h2 class="text-lg font-semibold text-gray-800 mb-3">Detail</h2>
                                    <div id="detailContent" class="text-sm text-gray-700">
                                        <!-- Isi detail akan dimasukkan dengan JavaScript -->
                                    </div>
                                    <button id="closePopup"
                                        class="mt-4 w-full bg-red-500 text-white py-2 rounded-lg hover:bg-red-600">Tutup</button>
                                </div>
                            </div>
                        @endforeach
                    </tbody>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll("a[href^='/detail-kayu/'], a[href^='/detail-bibit/']").forEach(
                link => {
                    link.addEventListener("click", function(event) {
                        event.preventDefault(); // Mencegah navigasi halaman
                        const id = this.getAttribute("href").split("/").pop();

                        // Ambil detail dari dataset atau bisa diganti dengan AJAX call
                        let detailData = getDetailById(id); // Simulasikan data

                        // Masukkan data ke dalam modal
                        detailContent.innerHTML = `
                    <p><strong>ID:</strong> ${detailData.id}</p>
                    <p><strong>Jenis:</strong> ${detailData.jenis}</p>
                    <p><strong>Jumlah:</strong> ${detailData.jumlah}</p>
                    <p><strong>Lokasi:</strong> ${detailData.lokasi}</p>
        ...
        // Fungsi simulasi data, bisa diganti dengan fetch API
